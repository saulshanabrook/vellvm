...
(* MARK: What happens if we move itree instances to iff instead of flip impl? *)
Global Instance euttG_cong_eutt {E R1 R2 RR} rH gH:
  Proper (eutt eq ==> eutt eq ==> iff)
         (@euttG E R1 R2 RR rH rH rH gH).
Proof.
  repeat intro; split; intros; eapply euttG_transU; econstructor; auto_ctrans_eq; symmetry; eauto.
Qed.

(* Not this one, since it is not symmetric *)
...
Global Instance euttG_cong_eq {E R1 R2 RR} rH rL gL gH:
  Proper (eq_itree eq ==> eq_itree eq ==> iff)
         (@euttG E R1 R2 RR rH rL gL gH).
Proof.
  repeat intro; split; intros; eapply euttG_cong_euttge; eauto; apply eq_sub_euttge; eauto; symmetry; eauto.
Qed.

Lemma eutt_cong_eutt_aux {E R1 R2 RR}:
  Proper (eutt eq ==> eutt eq ==> impl)
         (@eqit E R1 R2 RR true true).
Proof.
  einit. intros.
  (* And here is the drama: this previously instantaneous rewrite now takes 5.5s
     Why is it _so_ bad?
   *)
  Time rewrite <- H0.
...
Proof.
  ...
  eapply eutt_cong_eutt_aux; eauto. 
  eapply eutt_cong_eutt_aux'; eauto. 
Qed.

Lemma eutt_cong_eq_aux {E R1 R2 RR}:
...
Proof.
  split; intros.
  eapply eutt_cong_eq_aux; eauto. 
  eapply eutt_cong_eq_aux'; eauto. 
Qed.

Global Instance eutt_cong_euttH' {E R1 R2 RR} :
