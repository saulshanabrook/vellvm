...
From Vir Require Import LLVMAst Util AstLib Syntax.CFG Syntax.TypeUtil Semantics.TopLevel.
...
Section ShowInstances.
  ...
  Fixpoint show_exp (v : exp typ) :=
      ...
      | EXP_Cstring s => "unimplemented cstring" 
      | EXP_Undef => "undef"
...
Section ExpGenerators.
  ...
  Definition gen_non_zero_exp_size (sz : nat) (t : typ) : GenLLVM (exp typ)
    := match t with
       | TYPE_I n => ret EXP_Integer <*> lift gen_non_zero 
       | _ => lift failGen
       end.
